	...
	public static boolean aggregateStatistics(Timestamp startTime, Timestamp endTime, int newGranularity) {
		...
		new EAVStats()
				...
				.custom("CREATE TEMP TABLE" + 
						...
						+" (TIME TIMESTAMP, FK_ID_DATE INT, FK_ID_ENTITY INT, FK_ID_VALUES NUMERIC ARRAY, COUNT NUMERIC(64,6), MIN NUMERIC(64,6), AVG NUMERIC(64,6), MAX NUMERIC(64,6), SUM NUMERIC(64,6), NORM NUMERIC(64,6), GRANULARITY INT);")
				.execute();
		...
		success &=  new EAVStats()
				.queryCache(CFWDBEAVStats.class, "aggregateStatistics"+(cacheCounter++))
				.custom(" INSERT INTO CFW_EAV_STATS (TIME, FK_ID_DATE, FK_ID_ENTITY, FK_ID_VALUES, COUNT, MIN, AVG, MAX, SUM, NORM, GRANULARITY)" + 
