...
import com.xresch.cfw.features.query.store.CFWStoredQueryEditorGroupsMap.CFWStoredQueryEditorGroupsMapFields;
 ...
 **************************************************************************************************************/
public class CFWDBStoredQueryEditorGroupsMap {

	private static final String TABLE_NAME = new CFWStoredQueryEditorGroupsMap().getTableName();
	
	private static final Logger logger = CFWLog.getLogger(CFWDBStoredQueryEditorGroupsMap.class.getName());
	...
	public static boolean migrateOldStructure(CFWStoredQuery storedQuery) {
		...
		ResultSet result = new CFWSQL(new CFWStoredQuery())
			.select(CFWStoredQuery.FIELDNAME_EDITOR_GROUPS)
		...
		try {
			if(result.next()) {
				editorsJSONString = result.getString(CFWStoredQuery.FIELDNAME_EDITOR_GROUPS);
			}
	...
	public static boolean assignGroupToStoredQuery(Role role, CFWStoredQuery storedQuery) {
		...
		if(checkIsGroupAssignedToStoredQuery(role, storedQuery)) {
			new CFWLog(logger)
				.warn("The storedQuery '"+storedQuery.name()+"' is already assigned to role '"+role.name()+"' to allow editing.");
			return false;
		}
		
		String insertRoleSQL = "INSERT INTO "+TABLE_NAME+" ("
				  + CFWStoredQueryEditorGroupsMapFields.FK_ID_ROLE +", "
				  + CFWStoredQueryEditorGroupsMapFields.FK_ID_STOREDQUERY
				  + ") VALUES (?,?);";
		
		new CFWLog(logger).audit(CFWAuditLogAction.UPDATE, CFWStoredQueryEditorGroupsMap.class, "Add Role to StoredQuery: "+storedQuery.name()+", Role: "+role.name());
	...
	public static boolean updateGroupStoredQueryAssignments(CFWStoredQuery storedQuery, LinkedHashMap<String,String> rolesKeyLabel) {
		...
		if(!wasStarted) { CFW.DB.transactionStart(); }
			...
			boolean hasCleared = new CFWSQL(new CFWStoredQueryEditorGroupsMap())
						.delete()
						.where(CFWStoredQueryEditorGroupsMapFields.FK_ID_STOREDQUERY, storedQuery.id())
						.executeDelete();
			
			if(hasCleared) {
				new CFWLog(logger).audit(CFWAuditLogAction.CLEAR, CFWStoredQueryEditorGroupsMap.class, "Update Editor Group Assignments: "+storedQuery.name());
			}
	...
	public static boolean removeGroupFromStoredQuery(Role role, CFWStoredQuery storedQuery) {
		...
		new CFWLog(logger).audit(CFWAuditLogAction.UPDATE, CFWStoredQueryEditorGroupsMap.class, "Remove Role from StoredQuery: "+storedQuery.name()+", Role: "+role.name());
	...
	public static boolean checkIsGroupAssignedToStoredQuery(int roleid, int storedQueryid) {
		...
		return 0 != new CFWSQL(new CFWStoredQueryEditorGroupsMap())
			.queryCache()
			.selectCount()
			.where(CFWStoredQueryEditorGroupsMapFields.FK_ID_ROLE.toString(), roleid)
			.and(CFWStoredQueryEditorGroupsMapFields.FK_ID_STOREDQUERY.toString(), storedQueryid)
			.executeCount();
...
//	 ****************************************************************/
	public static LinkedHashMap<String, String> selectGroupsForStoredQueryAsKeyLabel(Integer storedQueryID) {
		...
		String query = 
				...
				+ " LEFT JOIN "+CFWStoredQueryEditorGroupsMap.TABLE_NAME+" M ON M.FK_ID_ROLE = U.PK_ID\r\n"
