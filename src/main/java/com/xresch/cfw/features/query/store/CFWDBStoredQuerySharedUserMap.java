...
import com.xresch.cfw.features.query.store.CFWStoredQuerySharedUserMap.CFWStoredQuerySharedUserMapFields;
 ...
 **************************************************************************************************************/
public class CFWDBStoredQuerySharedUserMap {

	private static final String TABLE_NAME = new CFWStoredQuerySharedUserMap().getTableName();
	
	private static final Logger logger = CFWLog.getLogger(CFWDBStoredQuerySharedUserMap.class.getName());
	...
	public static boolean migrateOldStructure(CFWStoredQuery storedQuery) {
		...
		ResultSet result = new CFWSQL(new CFWStoredQuery())
			.select(CFWStoredQuery.FIELDNAME_SHARE_WITH_USERS)
		...
		try {
			if(result.next()) {
				jsonString = result.getString(CFWStoredQuery.FIELDNAME_SHARE_WITH_USERS);
			}
	...
	public static boolean assignUserToStoredQuery(User user, CFWStoredQuery storedQuery) {
		...
		if(storedQuery == null) {
			new CFWLog(logger)
				.warn("Org StoredQuery cannot be null.");
			return false;
		}
		...
		String insertUserSQL = "INSERT INTO "+TABLE_NAME+" ("
				  + CFWStoredQuerySharedUserMapFields.FK_ID_USER +", "
				  + CFWStoredQuerySharedUserMapFields.FK_ID_STOREDQUERY
				  + ") VALUES (?,?);";
		
		new CFWLog(logger).audit(CFWAuditLogAction.UPDATE, CFWStoredQuerySharedUserMap.class, "Add User to StoredQuery: "+storedQuery.name()+", User: "+user.username());
	...
	public static boolean updateUserStoredQueryAssignments(CFWStoredQuery storedQuery, LinkedHashMap<String,String> usersKeyLabel) {
		...
		if(!wasStarted) { CFW.DB.transactionStart(); }
			...
			boolean hasCleared = new CFWSQL(new CFWStoredQuerySharedUserMap())
						.delete()
						.where(CFWStoredQuerySharedUserMapFields.FK_ID_STOREDQUERY, storedQuery.id())
						.executeDelete();
			
			if(hasCleared) {
				new CFWLog(logger).audit(CFWAuditLogAction.CLEAR, CFWStoredQuerySharedUserMap.class, "Update Shared User Assignments: "+storedQuery.name());
			}
	...
	public static boolean removeUserFromStoredQuery(User user, CFWStoredQuery storedQuery) {
		...
		new CFWLog(logger).audit(CFWAuditLogAction.UPDATE, CFWStoredQuerySharedUserMap.class, "Remove User from StoredQuery: "+storedQuery.name()+", User: "+user.username());
	...
	public static boolean checkIsUserAssignedToStoredQuery(int userid, int storedQueryid) {
		...
		return 0 != new CFWSQL(new CFWStoredQuerySharedUserMap())
			.queryCache()
			.selectCount()
			.where(CFWStoredQuerySharedUserMapFields.FK_ID_USER.toString(), userid)
			.and(CFWStoredQuerySharedUserMapFields.FK_ID_STOREDQUERY.toString(), storedQueryid)
			.executeCount();
...
//	 ****************************************************************/
	public static LinkedHashMap<String, String> selectUsersForStoredQueryAsKeyLabel(Integer storedQueryID) {
		...
		String query = 
				...
				+ " LEFT JOIN "+CFWStoredQuerySharedUserMap.TABLE_NAME+" M ON M.FK_ID_USER = U.PK_ID\r\n"
