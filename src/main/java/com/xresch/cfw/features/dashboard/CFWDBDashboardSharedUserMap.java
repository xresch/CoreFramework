...
import com.xresch.cfw.features.dashboard.DashboardSharedUserMap.DashboardSharedUserMapFields;
 ...
 **************************************************************************************************************/
public class CFWDBDashboardSharedUserMap {

	private static final String TABLE_NAME = new DashboardSharedUserMap().getTableName();
	
	private static final Logger logger = CFWLog.getLogger(CFWDBDashboardSharedUserMap.class.getName());
	...
	public static boolean migrateOldStructure(Dashboard dashboard) {
		...
		ResultSet result = new CFWSQL(new Dashboard())
			.select(Dashboard.FIELDNAME_SHARE_WITH_USERS)
		...
		try {
			if(result.next()) {
				jsonString = result.getString(Dashboard.FIELDNAME_SHARE_WITH_USERS);
			}
	...
	public static boolean assignUserToDashboard(User user, Dashboard dashboard) {
		...
		if(dashboard == null) {
			new CFWLog(logger)
				.warn("Org Dashboard cannot be null.");
			return false;
		}
		...
		String insertUserSQL = "INSERT INTO "+TABLE_NAME+" ("
				  + DashboardSharedUserMapFields.FK_ID_USER +", "
				  + DashboardSharedUserMapFields.FK_ID_DASHBOARD
				  + ") VALUES (?,?);";
		
		new CFWLog(logger).audit(CFWAuditLogAction.UPDATE, DashboardSharedUserMap.class, "Add User to Dashboard: "+dashboard.name()+", User: "+user.username());
	...
	public static boolean updateUserDashboardAssignments(Dashboard dashboard, LinkedHashMap<String,String> usersKeyLabel) {
		...
		CFW.DB.transactionStart();
			...
			boolean hasCleared = new CFWSQL(new DashboardSharedUserMap())
						.delete()
						.where(DashboardSharedUserMapFields.FK_ID_DASHBOARD, dashboard.id())
						.executeDelete();
	...
	public static boolean removeUserFromDashboard(User user, Dashboard dashboard) {
		...
		new CFWLog(logger).audit(CFWAuditLogAction.UPDATE, DashboardSharedUserMap.class, "Remove User from Dashboard: "+dashboard.name()+", User: "+user.username());
	...
	public static boolean checkIsUserAssignedToDashboard(int userid, int dashboardid) {
		...
		return 0 != new CFWSQL(new DashboardSharedUserMap())
			.queryCache()
			.selectCount()
			.where(DashboardSharedUserMapFields.FK_ID_USER.toString(), userid)
			.and(DashboardSharedUserMapFields.FK_ID_DASHBOARD.toString(), dashboardid)
			.executeCount();
...
//	 ****************************************************************/
	public static LinkedHashMap<String, String> selectUsersForDashboardAsKeyLabel(Integer dashboardID) {
		...
		String query = 
				...
				+ " LEFT JOIN "+DashboardSharedUserMap.TABLE_NAME+" M ON M.FK_ID_USER = U.PK_ID\r\n"
