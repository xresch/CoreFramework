...
import com.xresch.cfw.features.dashboard.DashboardEditorGroupsMap.DashboardEditorGroupsMapFields;
 ...
 **************************************************************************************************************/
public class CFWDBDashboardEditorGroupsMap {

	private static final String TABLE_NAME = new DashboardEditorGroupsMap().getTableName();
	
	private static final Logger logger = CFWLog.getLogger(CFWDBDashboardEditorGroupsMap.class.getName());
	...
	public static boolean migrateOldStructure(Dashboard dashboard) {
		...
		ResultSet result = new CFWSQL(new Dashboard())
			.select(Dashboard.FIELDNAME_EDITOR_GROUPS)
		...
		try {
			if(result.next()) {
				editorsJSONString = result.getString(Dashboard.FIELDNAME_EDITOR_GROUPS);
			}
	...
	public static boolean assignGroupToDashboard(Role role, Dashboard dashboard) {
		...
		if(checkIsGroupAssignedToDashboard(role, dashboard)) {
			new CFWLog(logger)
				.warn("The dashboard '"+dashboard.name()+"' is already assigned to role '"+role.name()+"' to allow editing.");
			return false;
		}
		
		String insertRoleSQL = "INSERT INTO "+TABLE_NAME+" ("
				  + DashboardEditorGroupsMapFields.FK_ID_ROLE +", "
				  + DashboardEditorGroupsMapFields.FK_ID_DASHBOARD
				  + ") VALUES (?,?);";
		
		new CFWLog(logger).audit(CFWAuditLogAction.UPDATE, DashboardEditorGroupsMap.class, "Add Role to Dashboard: "+dashboard.name()+", Role: "+role.name());
	...
	public static boolean updateGroupDashboardAssignments(Dashboard dashboard, LinkedHashMap<String,String> rolesKeyLabel) {
		...
		if(!wasStarted) { CFW.DB.transactionStart(); }
			...
			boolean hasCleared = new CFWSQL(new DashboardEditorGroupsMap())
						.delete()
						.where(DashboardEditorGroupsMapFields.FK_ID_DASHBOARD, dashboard.id())
						.executeDelete();
	...
	public static boolean removeGroupFromDashboard(Role role, Dashboard dashboard) {
		...
		new CFWLog(logger).audit(CFWAuditLogAction.UPDATE, DashboardEditorGroupsMap.class, "Remove Role from Dashboard: "+dashboard.name()+", Role: "+role.name());
	...
	public static boolean checkIsGroupAssignedToDashboard(int roleid, int dashboardid) {
		...
		return 0 != new CFWSQL(new DashboardEditorGroupsMap())
			.queryCache()
			.selectCount()
			.where(DashboardEditorGroupsMapFields.FK_ID_ROLE.toString(), roleid)
			.and(DashboardEditorGroupsMapFields.FK_ID_DASHBOARD.toString(), dashboardid)
			.executeCount();
...
//	 ****************************************************************/
	public static LinkedHashMap<String, String> selectGroupsForDashboardAsKeyLabel(Integer dashboardID) {
		...
		String query = 
				...
				+ " LEFT JOIN "+DashboardEditorGroupsMap.TABLE_NAME+" M ON M.FK_ID_ROLE = U.PK_ID\r\n"
