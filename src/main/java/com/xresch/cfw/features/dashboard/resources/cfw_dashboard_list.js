...
function cfw_dashboardlist_createTabs(){
	...
	if(pillsTab.length == 0){
		...
		//--------------------------------
		// Archived Dashboard Tab	
		if( CFW.hasPermission('Dashboard Creator') ){
			list.append('<li class="nav-item"><a class="nav-link" id="tab-myarchived" data-toggle="pill" href="#" role="tab" onclick="cfw_dashboardlist_draw({tab: \'myarchived\'})"><i class="fas fa-folder-open mr-2"></i>Archive</a></li>');
		}
		...
		if(CFW.hasPermission('Dashboard Admin')){
			list.append(
				'<li class="nav-item"><a class="nav-link" id="tab-adminarchived" data-toggle="pill" href="#" role="tab" onclick="cfw_dashboardlist_draw({tab: \'adminarchived\'})"><i class="fas fa-folder-open mr-2"></i>Admin Archive</a></li>'
...
function cfw_dashboardlist_changeDashboardOwner(id){
	...
	CFW.ui.showModalMedium(
			CFWL("cfw_dashboardcommon_editDashboard","Edit Dashboard"), 
			formDiv, 
			"cfw_dashboardlist_draw(CFW_DASHBOARDLIST_LAST_OPTIONS)"
	);
 ...
 * Delete
 ******************************************************************/
function cfw_dashboardlist_archive(id, isarchived){
 ...
 * 
 ******************************************************************/
function cfw_dashboardlist_printMyArchived(data){
	cfw_dashboardlist_printDashboards(data, 'myarchived');
}

/******************************************************************
 * 
 ******************************************************************/
function cfw_dashboardlist_printAdminArchived(data){
	cfw_dashboardlist_printDashboards(data, 'adminarchived');
}
				...
				function (record, id){
					...
					|| (record.IS_EDITOR && record.ALLOW_EDIT_SETTINGS) ){
						...
						htmlString = '<button class="btn btn-warning btn-sm text-white" alt="Duplicate" title="Duplicate" '
							+'onclick="CFW.ui.confirmExecute(\'This will create a duplicate of <strong>\\\''+encodedName+'\\\'</strong> and add it to your dashboards.\', \'Do it!\', \'cfw_dashboardlist_duplicate('+id+');\')">'
			...
			function (record, id){
				...
				|| type == 'admindashboards'
					...
					htmlString += '<button class="btn '+color+' btn-sm" alt="'+title+'" title="'+title+'" '
						+'onclick="CFW.ui.confirmExecute(\''+confirmMessage+' <strong>\\\''+encodedName+'\\\'</strong>?\', \'Do it!\', \'cfw_dashboardlist_archive('+id+', '+!isArchived+');\')">'
						+ '<i class="fa '+icon+'"></i>'
						+ '</button>';
				}else{
					htmlString += '&nbsp;';
			...
			actionButtons.push(
				function (record, id){
					...
					return '<button class="btn btn-danger btn-sm" alt="Delete" title="Delete" '
							+'onclick="CFW.ui.confirmExecute(\'Do you want to delete the dashboard <strong>\\\''+encodedName+'\\\'</strong>?\', \'Delete\', \'cfw_dashboardlist_delete('+id+');\')">'
	...
	function(){
		...
		switch(options.tab){
			...
			case "shareddashboards":	CFW.http.getJSON(CFW_DASHBOARDLIST_URL, {action: "fetch", item: "shareddashboards"}, cfw_dashboardlist_printSharedDashboards);
										break;
			case "myarchived":			CFW.http.getJSON(CFW_DASHBOARDLIST_URL, {action: "fetch", item: "myarchived"}, cfw_dashboardlist_printMyArchived);
										break;	
			case "adminarchived":		CFW.http.getJSON(CFW_DASHBOARDLIST_URL, {action: "fetch", item: "adminarchived"}, cfw_dashboardlist_printAdminArchived);
										break;	
