...
import com.xresch.cfw.features.credentials.CFWCredentialsEditorGroupsMap.CFWCredentialsEditorGroupsMapFields;
 ...
 **************************************************************************************************************/
public class CFWDBCredentialsEditorGroupsMap {

	private static final String TABLE_NAME = new CFWCredentialsEditorGroupsMap().getTableName();
	
	private static final Logger logger = CFWLog.getLogger(CFWDBCredentialsEditorGroupsMap.class.getName());
	...
	public static boolean migrateOldStructure(CFWCredentials credentials) {
		...
		ResultSet result = new CFWSQL(new CFWCredentials())
			.select(CFWCredentials.FIELDNAME_EDITOR_GROUPS)
		...
		try {
			if(result.next()) {
				editorsJSONString = result.getString(CFWCredentials.FIELDNAME_EDITOR_GROUPS);
			}
	...
	public static boolean assignGroupToCredentials(Role role, CFWCredentials credentials) {
		...
		if(checkIsGroupAssignedToCredentials(role, credentials)) {
			new CFWLog(logger)
				.warn("The credentials '"+credentials.name()+"' is already assigned to role '"+role.name()+"' to allow editing.");
			return false;
		}
	...
	public static boolean updateGroupCredentialsAssignments(CFWCredentials credentials, LinkedHashMap<String,String> rolesKeyLabel) {
		...
		if(!wasStarted) { CFW.DB.transactionStart(); }
			...
			if(hasCleared) {
				new CFWLog(logger).audit(CFWAuditLogAction.CLEAR, CFWCredentialsEditorGroupsMap.class, "Update Editor Group Assignments: "+credentials.name());
			}
	...
	public static boolean checkIsGroupAssignedToCredentials(int roleid, int credentialsid) {
		...
		return 0 != new CFWSQL(new CFWCredentialsEditorGroupsMap())
			.queryCache()
			.selectCount()
