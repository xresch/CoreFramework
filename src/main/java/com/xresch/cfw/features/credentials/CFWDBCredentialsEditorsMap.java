 ...
 **************************************************************************************************************/
public class CFWDBCredentialsEditorsMap {

	private static final String TABLE_NAME = new CFWCredentialsEditorsMap().getTableName();
	
	private static final Logger logger = CFWLog.getLogger(CFWDBCredentialsEditorsMap.class.getName());
	...
	public static boolean migrateOldStructure(CFWCredentials credentials) {
		...
		ResultSet result = new CFWSQL(new CFWCredentials())
			.select(CFWCredentials.FIELDNAME_EDITORS)
		...
		try {
			if(result.next()) {
				editorsJSONString = result.getString(CFWCredentials.FIELDNAME_EDITORS);
			}
	...
	public static boolean updateUserCredentialsAssignments(CFWCredentials credentials, LinkedHashMap<String,String> usersKeyLabel) {
		...
		if(!wasStarted) { CFW.DB.transactionStart(); }
			...
			if(hasCleared) {
				new CFWLog(logger).audit(CFWAuditLogAction.CLEAR, CFWCredentialsEditorsMap.class, "Update Shared User Assignments: "+credentials.name());
			}
	...
	public static boolean checkIsUserAssignedToCredentials(int userid, int credentialsid) {
		...
		return 0 != new CFWSQL(new CFWCredentialsEditorsMap())
			.queryCache()
			.selectCount()
