...
import com.xresch.cfw.features.filemanager.CFWStoredFileEditorsMap.CFWStoredFileEditorsMapFields;
 ...
 **************************************************************************************************************/
public class CFWDBStoredFileEditorsMap {

	private static final String TABLE_NAME = new CFWStoredFileEditorsMap().getTableName();
	
	private static final Logger logger = CFWLog.getLogger(CFWDBStoredFileEditorsMap.class.getName());
	...
	public static boolean migrateOldStructure(CFWStoredFile storedfile) {
		...
		ResultSet result = new CFWSQL(new CFWStoredFile())
			.select(CFWStoredFile.FIELDNAME_EDITORS)
		...
		try {
			if(result.next()) {
				editorsJSONString = result.getString(CFWStoredFile.FIELDNAME_EDITORS);
			}
	...
	public static boolean assignUserToStoredFile(User user, CFWStoredFile storedfile) {
		...
		String insertUserSQL = "INSERT INTO "+TABLE_NAME+" ("
				  + CFWStoredFileEditorsMapFields.FK_ID_USER +", "
				  + CFWStoredFileEditorsMapFields.FK_ID_STOREDFILE
				  + ") VALUES (?,?);";
		
		new CFWLog(logger).audit(CFWAuditLogAction.UPDATE, CFWStoredFileEditorsMap.class, "Add User to Stored File: "+storedfile.name()+", User: "+user.username());
	...
	public static boolean updateUserStoredFileAssignments(CFWStoredFile storedfile, LinkedHashMap<String,String> usersKeyLabel) {
		...
		if(!wasStarted) { CFW.DB.transactionStart(); }
			...
			boolean hasCleared = new CFWSQL(new CFWStoredFileEditorsMap())
						.delete()
						.where(CFWStoredFileEditorsMapFields.FK_ID_STOREDFILE, storedfile.id())
						.executeDelete();
			
			if(hasCleared) {
				new CFWLog(logger).audit(CFWAuditLogAction.CLEAR, CFWStoredFileEditorsMap.class, "Update Shared User Assignments: "+storedfile.name());
			}
	...
	public static boolean removeUserFromStoredFile(User user, CFWStoredFile storedfile) {
		...
		new CFWLog(logger).audit(CFWAuditLogAction.UPDATE, CFWStoredFileEditorsMap.class, "Remove User from Stored File: "+storedfile.name()+", User: "+user.username());
	...
	public static boolean checkIsUserAssignedToStoredFile(int userid, int storedfileid) {
		...
		return 0 != new CFWSQL(new CFWStoredFileEditorsMap())
			.queryCache()
			.selectCount()
			.where(CFWStoredFileEditorsMapFields.FK_ID_USER.toString(), userid)
			.and(CFWStoredFileEditorsMapFields.FK_ID_STOREDFILE.toString(), storedfileid)
			.executeCount();
...
//	 ****************************************************************/
	public static LinkedHashMap<String, String> selectUsersForStoredFileAsKeyLabel(Integer storedfileID) {
		...
		String query = 
				...
				+ " LEFT JOIN "+CFWStoredFileEditorsMap.TABLE_NAME+" M ON M.FK_ID_USER = U.PK_ID\r\n"
