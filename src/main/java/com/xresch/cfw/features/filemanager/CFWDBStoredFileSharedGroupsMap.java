...
import com.xresch.cfw.features.filemanager.CFWStoredFileSharedGroupsMap.CFWStoredFileSharedGroupsMapFields;
 ...
 **************************************************************************************************************/
public class CFWDBStoredFileSharedGroupsMap {

	private static final String TABLE_NAME = new CFWStoredFileSharedGroupsMap().getTableName();
	
	private static final Logger logger = CFWLog.getLogger(CFWDBStoredFileSharedGroupsMap.class.getName());
	...
	public static boolean migrateOldStructure(CFWStoredFile storedfile) {
		...
		ResultSet result = new CFWSQL(new CFWStoredFile())
			.select(CFWStoredFile.FIELDNAME_SHARE_WITH_GROUPS)
		...
		try {
			if(result.next()) {
				editorsJSONString = result.getString(CFWStoredFile.FIELDNAME_SHARE_WITH_GROUPS);
			}
	...
	public static boolean assignGroupToStoredFile(Role role, CFWStoredFile storedfile) {
		...
		if(checkIsGroupAssignedToStoredFile(role, storedfile)) {
			new CFWLog(logger)
				.warn("The Stored File '"+storedfile.name()+"' is already shared with '"+role.name()+"'.");
			return false;
		}
		
		String insertRoleSQL = "INSERT INTO "+TABLE_NAME+" ("
				  + CFWStoredFileSharedGroupsMapFields.FK_ID_ROLE +", "
				  + CFWStoredFileSharedGroupsMapFields.FK_ID_STOREDFILE
				  + ") VALUES (?,?);";
		
		new CFWLog(logger).audit(CFWAuditLogAction.UPDATE, CFWStoredFileSharedGroupsMap.class, "Add Role to StoredFile: "+storedfile.name()+", Role: "+role.name());
	 ...
	 * @param roleID
	 * @param storedfileID
	 * @return return true if role was added or if role/storedfile did not exist, false if failed
	 * 
	 ********************************************************************************************/
	public static boolean assignGroupToStoredFile(int roleID, int storedfileID) {
		...
		if(storedfile == null) { return true; }
	...
	public static boolean updateGroupStoredFileAssignments(CFWStoredFile storedfile, LinkedHashMap<String,String> rolesKeyLabel) {
		...
		if(!wasStarted) { CFW.DB.transactionStart(); }
			...
			boolean hasCleared = new CFWSQL(new CFWStoredFileSharedGroupsMap())
						.delete()
						.where(CFWStoredFileSharedGroupsMapFields.FK_ID_STOREDFILE, storedfile.id())
						.executeDelete();
			
			if(hasCleared) {
				new CFWLog(logger).audit(CFWAuditLogAction.CLEAR, CFWStoredFileSharedGroupsMap.class, "Update Shared Role Assignments: "+storedfile.name());
			}
	...
	public static boolean removeGroupFromStoredFile(Role role, CFWStoredFile storedfile) {
		...
		new CFWLog(logger).audit(CFWAuditLogAction.UPDATE, CFWStoredFileSharedGroupsMap.class, "Remove Role from Stored File: "+storedfile.name()+", Role: "+role.name());
	...
	public static boolean checkIsGroupAssignedToStoredFile(int roleid, int storedfileid) {
		...
		return 0 != new CFWSQL(new CFWStoredFileSharedGroupsMap())
			.queryCache()
			.selectCount()
			.where(CFWStoredFileSharedGroupsMapFields.FK_ID_ROLE.toString(), roleid)
			.and(CFWStoredFileSharedGroupsMapFields.FK_ID_STOREDFILE.toString(), storedfileid)
			.executeCount();
...
//	 ****************************************************************/
	public static LinkedHashMap<String, String> selectGroupsForStoredFileAsKeyLabel(Integer storedfileID) {
		...
		String query = 
				...
				+ " LEFT JOIN "+CFWStoredFileSharedGroupsMap.TABLE_NAME+" M ON M.FK_ID_ROLE = U.PK_ID\r\n"
